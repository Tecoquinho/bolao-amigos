...
export function PredictionCard({ prediction }: PredictionCardProps) {
  ...
  return (
    <article
      className={`overflow-hidden rounded-2xl border-[2.5px] px-4 py-3 shadow-[3px_4px_0_#1a3a2f] ${
    ...
    >
      <div className="space-y-2.5">
        <div className="min-w-0 text-center">
          {isFinished ? (
            ...
            <div className="grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-2 overflow-hidden whitespace-nowrap text-[13px] font-black text-ink">
              <TeamLabel
                ...
                justify="end"
              />
              <span className="inline-flex w-4 justify-center text-center text-[17px] text-ink/45">x</span>
              <TeamLabel
                name={prediction.awayTeamName}
                fifaCode={prediction.awayTeamFifaCode}
                flagUrl={prediction.awayTeamFlagUrl}
          ...
          <p className="mt-0.5 text-center text-[11px] font-bold text-ink/60">{formatTime(prediction.startsAt)}</p>
        </div>
        <div className="space-y-1 text-center text-sm">
          <div>
            <p className="text-[9px] font-black uppercase tracking-[0.08em] text-ink/50">Palpite</p>
            <div className={`mt-0.5 inline-flex items-center justify-center font-black text-ink ${isFinished ? "text-[16px] leading-none" : "text-[14px]"}`}>
          ...
          <div className="flex items-end justify-between gap-3">
            <div className="space-y-2 text-left text-sm">
              ...
              {isFinished ? (
                <p className="inline-flex rounded-full border-2 border-[#2d6a4f] bg-[#dcefdc] px-2.5 py-1 text-[12px] font-black text-[#1b4332]">
                  +{prediction.pointsAwarded} pts
                </p>
              ) : null}
            </div>
            <div className={isFinished ? "self-end pb-0.5" : "self-end pt-2 pb-0.5"}>
